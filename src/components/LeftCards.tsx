...
const LeftCards: React.FC<CardStackProps> = ({ cardNumber }) => {
    const cardWidth = 100; // 假定卡牌宽度为100px
                        ...
                        <div className="flex justify-center space-x-[-20px]"> {/* 使用负margin来实现卡牌的部分覆盖 */}
                            {Array.from({ length: cardNumber }, (_, index) => (
                                <div key={index} className="z-10" style={{ zIndex: cardNumber - index }}>
                                    <img src="/cards/back.svg" width={cardWidth} height={100} alt={`Card ${index + 1}`} />
                                </div>
                            ))}
                        </div>
                    </div>
                    <div className="relative text-white px-6 pb-6 mt-6">
                        <span className="block opacity-80 -mb-1"></span>
                    </div>
                </div>
            </div>
        </div>
